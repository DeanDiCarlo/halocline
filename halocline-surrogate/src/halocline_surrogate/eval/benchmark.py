...
def surrogate_milliseconds_per_scenario(
    *,
    ...
    model.eval()
    loader = DataLoader(HaloclineDataset(dataset_path), batch_size=batch_size)
    sample_count = 0
...
def main() -> None:
    ...
    batch1 = surrogate_milliseconds_per_scenario(
        dataset_path=args.dataset,
        checkpoint_path=args.checkpoint,
        batch_size=1,
        device_name=args.device,
    )
    batch64 = surrogate_milliseconds_per_scenario(
        dataset_path=args.dataset,
        checkpoint_path=args.checkpoint,
        batch_size=64,
        device_name=args.device,
    ...
    print(
        {
            "oracle_ms_per_scenario": oracle,
            "surrogate_ms_per_scenario_batch1": batch1,
            "surrogate_ms_per_scenario_batch64": batch64,
        }
    )
